...
public class JvmResearch   {
	...
	public static void main(String[] args) throws Exception{
		...
		try {m = new Mongo( "localhost" , 27017 ); } catch (UnknownHostException e) {e.printStackTrace();}
		
		// this is just a database I have on my system with a lot of records for a data source for testing purposes
		DB db = m.getDB("collector-actionfigures-mainpage");
		DBCollection fileredColl = db.getCollection("Terminator_Brand_Filtered");
