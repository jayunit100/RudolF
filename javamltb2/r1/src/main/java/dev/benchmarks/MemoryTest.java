...
public class MemoryTest extends BenchMarkBase {
	...
	public void genericCollectionAnalyzer() throws ClassNotFoundException, InstantiationException, IllegalAccessException {
	...
	// we do this because later this method can be extended to analyze class types passed in as a parameter
	Class clazz = Class.forName ("java.util.Hashtable");
	
	// get the initial memory using runtime and xbeans
	initialMemoryXBean = ManagementFactory.getMemoryMXBean().getHeapMemoryUsage().getUsed();
	initialMemoryRunTime = Runtime.getRuntime().totalMemory() - Runtime.getRuntime().freeMemory();
	
	
	// Create a primer instance of the class before your measure the heap the first time
	Object primer = clazz.newInstance();
	 for (int i = 0; i < objects.length; i++) {
         objects[i] = clazz.newInstance();
      }
	// get the final  memory using runtime and xbeans
	 usedMemoryXBean = ManagementFactory.getMemoryMXBean().getHeapMemoryUsage().getUsed();
	 usedMemoryRunTime = Runtime.getRuntime().totalMemory() - Runtime.getRuntime().freeMemory(); 
	...
	// simple routine to calculate an average size for 100 objects
	 ...
	 System.out.println("size of empty hashtable by XBean = "+approxSizeXBean+" size of empty hashtable by RunTime = "+ approxSizeRunTime );
