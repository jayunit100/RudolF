...
(defn ll [regex] "gets a list"
  (let [ll (-> ;;forward args as the first to each 
...
;;1 Partial functions : by sending the 1st arg to a function, you can then "complete" the function w/ 2nd arg in a map ;;2 Clojures facility for filtering maps by key "select-keys" 
...
(defn tomat [l] "listtomatrix"
  ...
  (let [kv (map (partial zipmap [:a :p :b :P :d :e :C :f :k :R]) l)] ;;creates partial func, applies each el of list to the func to fulfill it. 
   (map #(select-keys % [:P :C :R]) kv))) ;;return the filtered / selected map 
